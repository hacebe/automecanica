...
if(isset($_GET['module']) ){
	...
	switch($module){		
		...
		case "getLancamentos":
			...
			$result = $mysqli->query("			
				SELECT
				l.*, count(a.id) as anexos,  if(p.nome = \"Pro-Labore\" OR p.nome = \"Lucros Distribuidos\", IF(s.empresa_id = l.empresa_id, s.nome, null), IF(f.empresa_id = l.empresa_id, f.nome, null)) as favorecido_nome, if(p.nome != null or t.descricao != null, if(t.contabil = l.natureza_financeira, t.descricao, p.nome), if(p.cod = l.natureza_financeira, p.nome, t.descricao)) as natureza_titulo
				FROM 
